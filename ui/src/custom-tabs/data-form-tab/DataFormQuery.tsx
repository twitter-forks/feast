...
const DataFormQuery = (field: any, val: any) => {
  const url = window.location.href;
  console.log(url);
  const data_url = "http://0.0.0.0:8000/api/change_metadata" 
  const data = {'url': url, 'field': field, 'val': val}
  return fetch(data_url, {method: "POST", headers: {'content-type': 'application/json'}, body: JSON.stringify(data)}).then((res) => res.json())
};

export default DataFormQuery;
